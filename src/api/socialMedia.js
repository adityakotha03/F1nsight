  ...
  while (retryAttempt <= OPENF1_MAX_RETRIES) {
    ...
    try {
      ...
      if (directResponse.status === 429 && retryAttempt < OPENF1_MAX_RETRIES) {
        await sleep(OPENF1_RETRY_BASE_DELAY_MS * Math.pow(2, retryAttempt));
        retryAttempt += 1;
        continue;
      }
...
export async function getSocialDriverStandings(year = getCurrentYear()) {
  try {
...
export async function getSocialConstructorStandings(year = getCurrentYear()) {
  try {
