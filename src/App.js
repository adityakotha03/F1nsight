...
function LatestRaceRedirect() {
  ...
  useEffect(() => {
    ...
    const loadMostRecentRace = async () => {
      try {
        const race = await fetchMostRecentRace(currentYear);
        const meetingKey = race?.meetingKey;
        if (isMounted && meetingKey && meetingKey !== 'unknown') {
          setLatestRaceId(String(meetingKey));
        }
      } catch (error) {
        console.error('Failed to load most recent race:', error);
      } finally {
        if (isMounted) {
          setIsLoading(false);
        }
  ...
  return <Navigate to={`/race/${latestRaceId}`} replace />;
}
...
function MainContent({ setSelectedYear, selectedYear, resultPage, resultPagePath, setResultPagePath }) {
  ...
  return (
    <div className="h-full grow">
      ...
      <Routes>
        <Route path="/" element={<LatestRaceRedirect />} />
        <Route path="/about-us" element={<LandingPage2025 setResultPagePath={setResultPagePath} />} />
